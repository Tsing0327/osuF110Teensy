  ...
  if(flagStop == false){
    ...
    chatter.publish( &str_msg );
      ...
      if(pwm.pwm_drive < pwm_lowerlimit){
        analogWrite(speed_pin,pwm_lowerlimit);    //  Safety lower limit        
      } else if(pwm.pwm_drive > pwm_upperlimit){
        analogWrite(speed_pin,pwm_upperlimit);    //  Safety upper limit
      } else{
        analogWrite(speed_pin,pwm.pwm_drive);     //  Incoming data                    
      }

    
      if(pwm.pwm_angle < pwm_lowerlimit){
        analogWrite(sterring_pin,pwm_lowerlimit);    //  Safety lower limit        
      } else if(pwm.pwm_angle > pwm_upperlimit){
        analogWrite(sterring_pin,pwm_upperlimit);    //  Safety upper limit
      } else{
        analogWrite(sterring_pin,pwm.pwm_angle);     //  Incoming data                    
      }
  } else{
      analogWrite(speed_pin,pwm_center_value);
      analogWrite(sterring_pin,pwm_center_value);    
  }
}
...
void setup() {
  analogWriteFrequency(speed_pin, 100);
  analogWriteFrequency(sterring_pin, 100);
